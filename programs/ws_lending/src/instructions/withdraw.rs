...
pub fn process_withdraw(ctx: Context<Withdraw>,amount_to_withdraw:u64) -> Result<()> {
    ...
    let shares_to_withdraw=(amount_to_withdraw as f64/bank.total_deposits as f64)*bank.total_deposits_share as f64;
    let user = &mut ctx.accounts.user_account;
    if ctx.accounts.mint.to_account_info().key()==user.usdc_mint_address{
        ...
        user.deposited_usdc_share=user.deposited_usdc_share.checked_sub(shares_to_withdraw as u64).unwrap();
    }
    bank.total_deposits=bank.total_deposits.checked_sub(amount_to_withdraw).unwrap();
    bank.total_deposits_share=bank.total_deposits_share.checked_sub(shares_to_withdraw as u64).unwrap();
    Ok(())
}